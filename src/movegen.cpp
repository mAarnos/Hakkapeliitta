...
void MoveGen::generatePseudoLegalQuietChecks(const Position& pos, MoveList& moves)
{
    pos.getSideToMove() ? generatePseudoLegalQuietChecks<true>(pos, moves)
                        : generatePseudoLegalQuietChecks<false>(pos, moves);
}
...
void MoveGen::generatePseudoLegalQuietChecks(const Position& pos, MoveList& moves)
{
    int from;
    auto occupied = pos.getOccupiedSquares();
    auto targetBitboard = ~occupied;
    auto opponentKingSquare = Bitboards::lsb(pos.getBitboard(!side, Piece::King));
    auto bishopCheckSquares = Bitboards::bishopAttacks(opponentKingSquare, occupied);
    auto rookCheckSquares = Bitboards::rookAttacks(opponentKingSquare, occupied);
    auto dcCandidates = pos.getDiscoveredCheckCandidates();
    auto tempPiece = pos.getBitboard(side, Piece::Pawn) & ~(side ? Bitboards::ranks[1] : Bitboards::ranks[6]);

    // A pawn push discovered check can be generated by a pawn only when the pawn is not on the same file as the opposing king.
    const auto discoveredMask = (dcCandidates & ~Bitboards::files[file(opponentKingSquare)]);
    auto tempMove = (side ? (tempPiece & discoveredMask) >> 8 : (tempPiece & discoveredMask) << 8) & targetBitboard;
    addPawnSingleMovesFromMask<side>(moves, tempMove, false);
    tempMove = (side ? (tempMove & Bitboards::ranks[5]) >> 8 : (tempMove & Bitboards::ranks[2]) << 8) & targetBitboard;
    addPawnDoubleMovesFromMask<side>(moves, tempMove);

    tempMove = (side ? (tempPiece & ~discoveredMask) >> 8 : (tempPiece & ~discoveredMask) << 8) & targetBitboard;
    addPawnSingleMovesFromMask<side>(moves, tempMove & Bitboards::pawnAttacks(!side, opponentKingSquare), false);
    tempMove = (side ? (tempMove & Bitboards::ranks[5]) >> 8 : (tempMove & Bitboards::ranks[2]) << 8) & targetBitboard;
    addPawnDoubleMovesFromMask<side>(moves, tempMove & Bitboards::pawnAttacks(!side, opponentKingSquare));
    ...
    while (tempPiece)
    {
        ...
        if (!Bitboards::testBit(dcCandidates, from))
        {
            tempMove &= Bitboards::knightAttacks(opponentKingSquare);
        }
        addPieceMovesFromMask(moves, tempMove, from);
    }
    ...
    while (tempPiece)
    {
        ...
        if (!Bitboards::testBit(dcCandidates, from))
        {
            tempMove &= bishopCheckSquares | rookCheckSquares;
        }
        addPieceMovesFromMask(moves, tempMove, from);
    }
    ...
    // TODO: add castling here someday.
    from = Bitboards::lsb(pos.getBitboard(side, Piece::King));
    tempMove = Bitboards::testBit(dcCandidates, from) ? (Bitboards::kingAttacks(from) & targetBitboard & ~Bitboards::lineFormedBySquares(from, opponentKingSquare)) : 0;
    addPieceMovesFromMask(moves, tempMove, from);
}
